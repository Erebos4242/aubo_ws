...
class RobotSim(object):
    ...
    def get_position(self, img):
        ...
        for index in range(len(contours)):
            ...
            res += [rotation, target_width]
            object_position += list(res)
            cv2.putText(image, "(%.3f, %.3f, %.3f, %.3f)" % (res[0], res[1], res[2], res[3]),
    ...
    def grasp_excution(self, data):
        print(time(), 'grasp_excution start')
        with self.lock:
            self.init_joint_state()
        target = data.grasp_position
        with self.lock:
            self.go_to_pose_goal(target[0], target[1], target[2] + 0.17, target[3])
        with self.lock:
            self.grasp(target[3])
        with self.lock:
            self.place()
