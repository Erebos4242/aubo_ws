...
class RobotSim(object):
    ...
    def init_joint_state(self):
        if not self.robot_ready:
            print(time(), "robot not ready, init joint failed")
            return
    ...
    def go_to_pose_goal(self, x, y, z, r):
        if not self.robot_ready:
            print(time(), "robot not ready, go to pose goal failed")
            return
        self.robot_ready = False
        self.robot_init = False
    ...
    def grasp(self, data):
        if not self.graper_ready:
            print(time(), "graper not ready, grasp failed")
            return
        self.graper_ready = False
        data = 0.9 / 0.1 * data
        position = Float64()
